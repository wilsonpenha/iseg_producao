...
function salvarRepasse(){
	submitFormAjax('#formRepasseComissao', getContextApp()+'/propostaTransportes/salvarRepasse.action', '#divRepasseComissao');
}
...
function alterarGrupoDeVendaSemSalvar(){
	...
	submitFormAjax('#formLookup', getContextApp()+'/propostaTransportes/alterarGrupoDeVendaSemSalvar.action', '#divRepasseComissao');
}
...
function inserirComissionadoSemSalvar(){
	if(validaComissionadoComissao()){
		...
		submitFormAjaxAllArgs('#formComissionados', getContextApp()+'/propostaTransportes/inserirComissionadoSemSalvar.action', null, null, null, null, function(response){$('#tableComissionadosRepasse tbody').append(response);});
...
function abrirDivRepasse(parcelaSelecionada, parcelasPropostaId,listIndex){
	...
	submitFormAjax('#formRepasseComissao', getContextApp()+'/propostaTransportes/repassePorParcela.action', '#divRepasse', '#loadProgress');
	$('#repasseComissao').modal({onOpen: openModalRepasse, close: false});
}

function recarregarParcelas(){
	$("#gpPropostaId").val($('#propostaId').val());
	submitFormAjax('#formLookup', getContextApp()+'/propostaTransportes/carregarParcelas.action', '#divParcelas', '#loadProgress');
	$('#repasseComissao').modal({onOpen: openModalRepasse, close: false});
}
