...
function salvarRepasse(){
	submitFormAjax('#formRepasseComissao', getContextApp()+'/propostaImoveisFatura/salvarRepasse.action', '#divRepasseComissao');
}
...
function alterarGrupoDeVendaSemSalvar(){
	...
	submitFormAjax('#formLookup', getContextApp()+'/propostaImoveisFatura/alterarGrupoDeVendaSemSalvar.action', '#divRepasseComissao');
}
...
function inserirComissionadoSemSalvar(){
	if(validaComissionadoComissao()){
		...
		submitFormAjaxAllArgs('#formComissionados', getContextApp()+'/propostaImoveisFatura/inserirComissionadoSemSalvar.action', null, null, null, null, function(response){$('#tableComissionadosRepasse tbody').append(response);});
...
function abrirDivRepasse(parcelaSelecionada, parcelasPropostaId,listIndex){
	...
	$('#divRepasseTitulo')[0].innerHTML = 'Repasse de Comissões - Referência: '+parcelaSelecionada.substring(3);		
	submitFormAjax('#formRepasseComissao', getContextApp()+'/propostaImoveisFatura/repassePorParcela.action', '#divRepasse', '#loadProgress');
	$('#repasseComissao').modal({onOpen: openModalRepasse, close: false});
}

function recarregarParcelas(){
	$("#gpPropostaId").val($('#propostaId').val());
	submitFormAjax('#formLookup', getContextApp()+'/propostaImoveisFatura/carregarParcelas.action', '#divParcelas', '#loadProgress');
	$('#repasseComissao').modal({onOpen: openModalRepasse, close: false});
}
