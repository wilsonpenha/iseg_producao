...
<input type="hidden" name="endosso.id" value="<s:property value="endosso.id" />"/>
<table>
  <tr>
	<td colspan="4">
...
<table class="tableTag" id="DivTable" width="100%" align="center"
	...
	<tr class="tableTag_row" id="linha-1" style="cursor: hand"
		...
		<td class="tableTag_cell" id="" width="10%" nowrap="1"><b>Nome</b><img
	...
	<s:if test="list.size > 0">
		<s:iterator value="list" status="rowstatus">
			...
			<td class="tableTag_cell" id="<s:property value="id"/>"
				onClick="javascript:var obj = new Object();if (document.all){obj.value=this.innerText.substring(this.innerText.indexOf('|')+1)}else{obj.value=this.textContent.substring(this.textContent.indexOf('|')+1)};document.formLista.elements[elemento_no_formulario2('endosso.id',document.formLista)].value = obj.value;document.formLista.action='editar.action';document.formLista.submit();"><s:property
				value="nome"/><span style="display: none">|<s:property value="id"/></span></td>
			...
			<td class="tableTag_cell" id="<s:property value="id"/>"
				onClick="javascript:var obj = new Object();if (document.all){obj.value=this.innerText.substring(this.innerText.indexOf('|')+1)}else{obj.value=this.textContent.substring(this.textContent.indexOf('|')+1)};document.formLista.elements[elemento_no_formulario2('endosso.id',document.formLista)].value = obj.value;document.formLista.action='editar.action';document.formLista.submit();">
				<s:if test="%{isAtivo}">
					Ativa
				</s:if>
				<s:else>
