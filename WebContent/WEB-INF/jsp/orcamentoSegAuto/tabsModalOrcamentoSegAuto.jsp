...
<div class="sub_top_page">
	  			...
	  			<td>
				     <fieldset>
						...
						<table width="100%" id="tableCoberturaContratada">
							<s:iterator value="orcamentoCoberturaIsegList" status="k">
								<tr>
									<td>
										<s:textfield name ="orcamentoSeguradora.orcamentoCoberturaCollection[%{#k.index}].orcamentoSeguradora.id" style="display:none"/>
										<s:textfield name ="orcamentoSeguradora.orcamentoCoberturaCollection[%{#k.index}].id" style="display:none"/>
										...
										<s:if test="%{coberturasList.size() > 0}">
											<s:select theme="simple" style="width:250px" name="orcamentoSeguradora.orcamentoCoberturaCollection[%{#k.index}].coberturas.id" headerKey=""  headerValue="-- Selecione --" list="coberturasList" listKey="id" listValue="descricao" value="%{orcamentoSeguradora.orcamentoCoberturaCollection[#k.index].coberturas.id}" onchange="calculoAlterado = true;"/>
										</s:if>
										<s:else>
											<s:select theme="simple" style="width:250px" name="orcamentoSeguradora.orcamentoCoberturaCollection[%{#k.index}].coberturasIseg.id" headerKey=""  headerValue="-- Selecione --" list="coberturasIsegList" listKey="id" listValue="descricao" value="%{orcamentoSeguradora.orcamentoCoberturaCollection[#k.index].coberturasIseg.id}" onchange="calculoAlterado = true;"/>
										</s:else>
									</td>
									<td></td>
									<td>
					...
					<fieldset>
						...
						<table width="100%" id="tableServicoContratado">
							<s:iterator value="orcamentoServicosIsegList" status="x">
							<tr>
								<td>
									<s:textfield name ="orcamentoSeguradora.orcamentoServicosCollection[%{#x.index}].orcamentoSeguradora.id" style="display:none"/>
									<s:textfield name ="orcamentoSeguradora.orcamentoServicosCollection[%{#x.index}].id" style="display:none"/>
									...
									<s:if test="%{servicosList.size() > 0}">
										<s:select theme="simple" style="width:250px" name="orcamentoSeguradora.orcamentoServicosCollection[%{#x.index}].servicos.id" headerKey=""  headerValue="-- NÃO CONTRATADO --" list="servicosList" listKey="id" listValue="descricao" value="%{orcamentoSeguradora.orcamentoServicosCollection[#x.index].servicos.id}" onchange="calculoAlterado = true;"/>
									</s:if>
									<s:else>
										<s:select theme="simple" style="width:250px" name="orcamentoSeguradora.orcamentoServicosCollection[%{#x.index}].servicosIseg.id" headerKey=""  headerValue="-- NÃO CONTRATADO --" list="servicosIsegList" listKey="id" listValue="descricao" value="%{orcamentoSeguradora.orcamentoServicosCollection[#x.index].servicosIseg.id}" onchange="calculoAlterado = true;"/>
									</s:else>
								</td>
								<td>
									<s:if test="%{#x.index == 0}">
  ...
  <jsp:include page="modalMensagensCalculoOnline.jsp" flush="false"></jsp:include>
