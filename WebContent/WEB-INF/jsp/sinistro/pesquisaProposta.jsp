...
<script language="javascript" charset="UTF-8" src="<s:url value="/common/wizardFinal/tab-panel.js?v=0" includeParams="none"/>"></script>
  ...
  <table width="100%" cellspacing="0" cellpadding="0" align="center" class="Main_Client_TableEdition">
	  ...
	  <td colspan="4">
		<table width="100%" class="FieldGroup" border="0" cellpadding="0" cellspacing="0">
		  ...
		  <tr>
			...
			<td class="FieldGroupTitle_MaxSize">Criar Sinistro/Pesquisa de Pedidos de Seguro</td>
	...
	<tr>
	  ...
	  <td class="Combo_ContentCell">
	    <s:select theme="simple" cssClass="dojoValidateValid" id="statusOperacionalId" name="propostaSearch.statusOperacional" list="statusOperacionalList" headerKey="" headerValue="- SELECIONE -" />
	  </td>
	</tr>
	<tr>
	  <td width="18%" class="TitleCell"><label class="LabelText" for="ramoId">Ramo</label></td>
	  <td width="32%" class="TextBox_ContentCell">
		<s:select theme="simple" cssClass="dojoValidateValid" id="ramoId" name="propostaSearch.ramo.id" list="ramoSubRamoList" listKey="id" listValue="nomeRamoSubRamo" headerKey="" headerValue="- SELECIONE -" />
	  </td>
		...
		<div align="center">
		  <table>
			<tr>
			  ...
			  <td>
				<input type="button" id="btnf3" style="display: none;" onclick="voltar();"/>
				<table onmouseout="this.className='wButton'; window.status=''; return true;" onmouseover="this.className='wButton_hover'; 	window.status='Entrar'; return true;" onmouseup="this.className='wButton'; return true;" onmousedown="this.className='wButton_active'; return true;" class="wButton"
					   onclick="document.getElementById('btnf3').click()">
				  <tr>
					<td class="wButton_left"></td>
					<td class="wButton_text wButton_width">F9-Voltar</td>
					<td class="wButton_right"/>
				  </tr>
				</table>
			  </td>
...
function voltar(){
	submitForm(document.formLista, '<s:url namespace="/sinistro" action="displayList.action" includeParams="none"/>', false);
}

function listaLista(){
	submitFormAjax('#formLista', '<s:url namespace="/preSinistro" action="listalista.action" includeParams="none"/>', '#divResult');
}

function listaNext(){
	submitFormAjax('#formLista', '<s:url namespace="/preSinistro" action="listanext.action" includeParams="none"/>', '#divResult');
}

function listaPrevious(){
	submitFormAjax('#formLista', '<s:url namespace="/preSinistro" action="listaprevious.action" includeParams="none"/>', '#divResult');
}

function listaLast(){
	submitFormAjax('#formLista', '<s:url namespace="/preSinistro" action="listalast.action" includeParams="none"/>', '#divResult');
}

function listaFirst(){
	submitFormAjax('#formLista', '<s:url namespace="/preSinistro" action="listafirst.action" includeParams="none"/>', '#divResult');
}
...
function criarNova(){
    ...
    if (document.getElementById("paramSeguradoraId").options[document.getElementById("paramSeguradoraId").selectedIndex].value == ""){
		error = "Selecione uma Seguradora!";
		document.getElementById("paramSeguradoraId").focus();
	}else if (document.getElementById("paramRamoId").options[document.getElementById("paramRamoId").selectedIndex].value == ""){
		error = "Selecione um Ramo!";
		document.getElementById("paramRamoId").focus();
	}

	if(error == ""){
		submitForm(document.paramForm, '<s:url action="novo.action" includeParams="none" />', false);
	}else{
		alert(error);
	}
}
