		...
		<table cellspacing="3" cellpadding="0" align="center" class="Main_Client_TableRelation">
		  ...
		  <tr>
			...
			<td width="30%" class="TextBox_ContentCell">
				<s:select cssClass="Combo requiredField" id="isDetalhado" name="isDetalhado" list="#{'0':'NORMAL', '1':'DETALHADO', '2':'SIMPLIFICADO'}"/>
			</td>
			<td class="TitleCell" align="left"><span class="LabelText">Situação</span></td>
		  	<td class="TextBox_ContentCell">
		  	    <s:textfield id="idRelatorio" name="idRelatorio" cssStyle="display:none"/>
			    <s:select cssClass="Combo requiredField" id="situacaoOperacional" name="situacaoOperacional" list="#{'0':'TODAS', '1':'VIGENTES/VENCIDAS'}"/>
		  	</td>
		  </tr>
...
function openWinReport(){
	...
	submitForm($('#formLista').get(0), getContextApp()+'/jasperreportRenovacao/runjasper.action', false, "report");
}

function validaDatas(){
	var retorno = true;
	
	if (($('#inicioTerminoVigencia').val() == '')) {
		alert('É obrigatório informar uma período inicial!');
		return;
	}
	
	if (($('#fimTerminoVigencia').val() == '')) {
