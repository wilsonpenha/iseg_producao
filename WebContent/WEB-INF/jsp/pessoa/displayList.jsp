...
<form id="formLista" name="formLista" method="post" namespace="/pessoa">
  <s:textfield theme="simple" id="associadoId" name="associadoId" cssStyle="display:none;"/>

  <s:submit id="submitButton" cssStyle="display:none" theme="simple"/>
	<input id="pessoaId" type="hidden" name="pessoa.id">
  ...
  <table width="100%" cellspacing="0" cellpadding="0" align="center" class="Main_Client_TableEdition">
	  ...
	  <td colspan="4">
		<table width="100%" class="FieldGroup" border="0" cellpadding="0" cellspacing="0">
		  ...
		  <tr>
			...
			<td class="FieldGroupTitle_MaxSize">Pesquisa de Pessoa</td>
	...
	<tr>
	  ...
	  <td width="32%" class="TextBox_ContentCell">
		...
		<s:textfield theme="simple" id="clienteId" name="pessoaSearch.id" cssStyle="display:none;"/>
		<s:textfield theme="simple" id="clienteNome" cssClass="TextBox_100percent_width" name="pessoaSearch.nome" />
	  </td>
	  <td width="38%">&nbsp;</td>
	</tr>
	<tr>
	  ...
	  <td width="32%" class="TextBox_ContentCell">
		<s:textfield theme="simple" id="clienteCpfCnpj" cssClass="dojoValidateValid TextBox_100percent_width" name="pessoaSearch.cpfOuCnpj" />
	  </td>
	  <td width="38%">&nbsp;</td>
	</tr>
	<tr>
...
<form id="formLookup" name="formLookup">
  ...
  <div id="paramPessoa" style="display:none" >
  <form id="paramFormPessoa" name="paramFormPessoa">
    <s:textfield theme="simple" id="associadoId" name="associadoId" cssStyle="display:none;"/>
  
	<input id="paramPessoaId" type="hidden" name="pessoaSearch.id"/>
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="Main_ContentPane_TabPane" id="tdtopTabs">
		<tr id="paramParentTabContainer">
			<td class="tabBackGroundInit" style="background-color: #505050"></td>
		</tr>
  ...
  <tr>
    <td colspan="2" class="TitleCell" style="text-align: left">
      <input type="radio" id="pf" name="tipoCliente" value="0" checked="checked"/>
  ...
  <tr>
    <td colspan="2" class="TitleCell" style="text-align: left">
      <input type="radio" id="pjComPerfil" name="tipoCliente" value="1"/>
  ...
  <tr>
  	<td><div align="center">
		<input type="button" disabled="disabled" id="btnf2" style="display: none;" onclick="criarNovaPessoa();"/>
  		<table onmouseout="this.className='wButton'; window.status=''; return true;" onmouseover="this.className='wButton_hover'; 	window.status='Entrar'; return true;" onmouseup="this.className='wButton'; return true;" onmousedown="this.className='wButton_active'; return true;" class="wButton"
  	...
  	<td><div align="center">
		<input type="button" class="modalClose" id="btnf9" style="display: none;" onclick="fechaParamNovaPessoa();"/>
  		<table onmouseout="this.className='wButton'; window.status=''; return true;" onmouseover="this.className='wButton_hover'; 	window.status='Entrar'; return true;" onmouseup="this.className='wButton'; return true;" onmousedown="this.className='wButton_active'; return true;" class="wButton"
...
$(document).ready(function () {
	$('#btnf3').click(function (e) {
		e.preventDefault();
		$('#paramPessoa').modal({onShow: modalShow, close: false});
	});
...
function criarNovaPessoa(){
    ...
    var actionName = 'novo';
    submitForm(document.paramFormPessoa, getContextApp()+'/pessoa/novo.action', false);
}

function listaLista(){
	if ($('#page').val())
		$('#page').val('0');
	submitFormAjax('#formLista', getContextApp()+'/pessoa/listalista.action', '#divResult');
}

function listaNext(){
	submitFormAjax('#formLista', getContextApp()+'/pessoa/listanext.action', '#divResult');
}

function listaPrevious(){
	submitFormAjax('#formLista', getContextApp()+'/pessoa/listaprevious.action', '#divResult');
}

function listaLast(){
	submitFormAjax('#formLista', getContextApp()+'/pessoa/listalast.action', '#divResult');
}

function listaFirst(){
	submitFormAjax('#formLista', getContextApp()+'/pessoa/listafirst.action', '#divResult');
}

function excluirPessoa(){
	if (ConfirmaDisplayList(document.formLista,'excluir')) {
		submitForm(document.formLista, getContextApp()+'/pessoa/excluir.action', false);
	}
}

function modalShow (dialog) {
	setTimeout(function(){paramNovaPessoa();},2000);
}

function paramNovaPessoa(){
...
jQuery(document).ready(function(){
	...
	$("#clienteNome").autocomplete('<s:url value="/objlookup/getPessoaList.action" includeParams="none" />', {idFieldsToUpdate:["#clienteId"],onEnterPress:"listaLista()",mustMatch:0,autoFill:false,  width:400,formatItemOnSelect:splitResultGetFirst});
	$("#clienteCpfCnpj").autocomplete('<s:url value="/objlookup/getPessoaByCpfOuCnpj.action" includeParams="none" />', {idFieldsToUpdate:["#clienteId"],onEnterPress:"listaLista()",mustMatch:0,autoFill:false,  width:400,formatItemOnSelect:splitResultGetFirst});

	addFunctionOnEnterPress(function(event){execOnEnterPress(event, 'listaLista()');}, 'clienteNome');
});

function showHideOpcoesNovaPessoa(){
	$('#paramTipo').val() == 'NOVA' ? $('#opcoesNovaPessoa').show() : $('#opcoesNovaPessoa').hide();
}
