...
%>
		...
		<tr>
			<td width="34%"><label for="agenciamentoGrupoDeVendaId">Produtor/Grupo:</label><br />
				<s:select theme="simple" disabled="true" id="grupoDeVendaId" name="agenciamento.grupoDeVenda.id" cssClass="dojoValidateValid" list="grupoDeVendaList" listKey="id" listValue="nomeDoGrupo" onchange="alterarGrupoDeVendaAgenciamentoSelecionadaSemSalvar();" />
			</td>
		...
		<tr>
			...
			<div id="divListaComissionadosAgenciamento">
			  ...
			  <table id="tableComissionadoAgenciamento" class="listItens">
				...
				<s:textfield theme="simple" id="comissionadoAgenciamentoCount" name="comissionadoAgenciamentoCount" cssStyle="display:none;" />
				<s:iterator value="comissionadoAgenciamentoList" status="stat">
				<tr style="height: 20px;" id="row_${stat.index}">
				  <td nowrap="nowrap" class="small_image">
					<img alt="" style="cursor:pointer;" src="<s:url value="/common/web/images/delete.gif" includeParams="none"/>" onclick="if(confirma('Está operação poderá gerar estornos de repasse caso haja baixa de comissão, Confirma exclusão?')){setAlterado(true);$('#row_${stat.index}').remove();}"/>
				  </td>
				  <td>
