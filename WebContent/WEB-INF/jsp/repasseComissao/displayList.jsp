...
<form id="formLista" name="formLista" method="post">
  ...
  <table width="100%" cellspacing="0" cellpadding="0" align="center" class="Main_Client_TableEdition">
	  ...
	  <td colspan="4">
		<table width="100%" class="FieldGroup" border="0" cellpadding="0" cellspacing="0">
		  ...
		  <tr>
			...
			<td class="FieldGroupTitle_MaxSize">Baixa de Repasse de Comissões</td>
	...
	<tr>
	  ...
	  <tr>
		<td width="20%" class="TitleCell"><span class="LabelText">Início (Data Extrato): </span></td>
		<td width="30%" class="TextBox_ContentCell">
		  <s:textfield theme="simple" cssClass="dojoValidateValid TextBox_OneCells_width" id="inicioDataPagamento" name="inicioDataPagamento" maxlength="15" onblur="isValidDate(this)"/>
		</td>
		<td width="20%" class="TitleCell"><span class="LabelText">Fim (Data Extrato): </span></td>
		<td width="30%" class="TextBox_ContentCell">
		  <s:textfield theme="simple" cssClass="dojoValidateValid TextBox_OneCells_width" id="fimDataPagamento" name="fimDataPagamento" maxlength="15" onblur="isValidDate(this)"/>
		</td>
	  </tr>
	  <tr>
		...
		<td width="20%" class="TitleCell" align="left"><span class="LabelText">Situação</span></td>
	  	<td width="30%" class="TextBox_ContentCell">
		    <s:select theme="simple" cssClass="Combo requiredField" id="situacao" name="situacao" list="#{'0':'A PAGAR'}"/>
	  	</td>
	  </tr>
	  <tr id="trDataPagamento" style="display: none">
		<td width="20%" class="TitleCell"><span class="LabelText">Data da Quitação: </span></td>
		<td width="30%" class="TextBox_ContentCell">
		  <s:textfield theme="simple" cssClass="dojoValidateValid TextBox_OneCells_width" id="dataQuitacao" name="dataQuitacao" maxlength="15" onblur="isValidDate(this)"/>
		</td>
		<td width="20%" class="TitleCell"><span class="LabelText">&nbsp; </span></td>
		<td width="30%" class="TextBox_ContentCell">
		  &nbsp;
		</td>
	  </tr>
		...
		<div align="center">
		  <table>
			<tr>
			  ...
			  <td><div id="botaoQuitar" style="display: none">
				<input type="button" id="btnf4" style="display: none;" onclick="efetivarQuitacao();"/>
				<table onmouseout="this.className='wButton'; window.status=''; return true;" onmouseover="this.className='wButton_hover'; 	window.status='Entrar'; return true;" onmouseup="this.className='wButton'; return true;" onmousedown="this.className='wButton_active'; return true;" class="wButton"
					   onclick="if(VerificaQtosCamposChecados(document.formLista.repasseIds)==0){alert('Marque pelo menos um lançamento para quitação!')}else{document.getElementById('btnf4').click()}">
				  <tr>
					<td class="wButton_left"></td>
					<td class="wButton_text wButton_width">F4-Quitar Repasse</td>
					<td class="wButton_right"/>
				  </tr>
				</table></div>
			  </td>
...
function listaLista(){
	...
	submitFormAjax('#formLista', '<s:url action="/listaquitacao.action" includeParams="none"/>', '#divResult');
}

function efetivarQuitacao(){
	if(isEmpty('#inicioDataPagamento')){
		$("#inicioDataPagamento").focus();
		alert("A data inicial deve ser informada!");
		return;
	...
	if(isEmpty('#dataQuitacao')){
		$("#dataQuitacao").focus();
		alert("A data quitação deve ser informada!");
		return;
	}
	if (confirm('Confirma a quitação dos repasse?')){
	    submitFormAjax('#formLista', '<s:url action="/efetivarQuitacao.action" includeParams="none"/>', '#divResult');
	}
}
...
jQuery(document).ready(function(){
	...
	MaskInput($('#inicioDataPagamento').get(0), "99/99/9999");
	MaskInput($('#fimDataPagamento').get(0), "99/99/9999");
