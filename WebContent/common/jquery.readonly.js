...
window.com_powelltechs_readonly = window.com_powelltechs_readonly || {
  ...
  _updateOverlay: function(el){  
    ...
    if (overlay[0].id){
        window.eval("objeto = window."+overlay[0].id);
        if (objeto!=null){
        	if (objeto.type=="overflow" || objeto.type=="underflow"){
            	if (objeto.type=="overflow"){
					switch (objeto.detail) {
					case 0:
	    		...
	    		if (horizontalOverflow && verticalOverflow)
	    			overlay.css('position', 'absolute').css('top', d.top-76).css('left', d.left-40).css('width', d.width-20).css('height', d.height-20).css('z-index', '100');
	    		else if (horizontalOverflow)
	    			overlay.css('position', 'absolute').css('top', d.top-76).css('left', d.left-40).css('width', d.width+2).css('height', d.height).css('z-index', '100');
	    		else if (verticalOverflow)
	    			overlay.css('position', 'absolute').css('top', d.top-76).css('left', d.left-40).css('width', d.width-20).css('height', d.height).css('z-index', '100');
	    		else
	    			overlay.css('position', 'absolute').css('top', d.top-76).css('left', d.left-40).css('width', d.width+2).css('height', d.height).css('z-index', '100');
        	}else{
	    		if (objeto.horizontalOverflow && objeto.verticalOverflow)
	    			overlay.css('position', 'absolute').css('top', d.top-76).css('left', d.left-40).css('width', d.width-20).css('height', d.height-20).css('z-index', '100');
	    		else if (objeto.horizontalOverflow)
	    			overlay.css('position', 'absolute').css('top', d.top-76).css('left', d.left-40).css('width', d.width+2).css('height', d.height).css('z-index', '100');
	    		else if (objeto.verticalOverflow)
	    			overlay.css('position', 'absolute').css('top', d.top-76).css('left', d.left-40).css('width', d.width-20).css('height', d.height).css('z-index', '100');
	    		else
	    			overlay.css('position', 'absolute').css('top', d.top-76).css('left', d.left-40).css('width', d.width+2).css('height', d.height).css('z-index', '100');
        	}
  ...
  setOverlay: function(el){
    ...
    var overlay = jQuery('<div id="divReadonly_'+currentDiv+'_event" class="readonly_overlay tabPane" style="visibility: hidden"></div>');
...
(function(jQuery) {
  ...
  jQuery.extend(jQuery.fn, {
    ...
    readonly : function(status, currDiv) {
