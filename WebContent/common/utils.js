...
function buttonMenuVisibility(id, visibility) {
	if(!top.frameMenu){
		...
		if(frameset.cols.substring(0,1)=="0" && visibility == 'hide'){
			frameset.cols = "25%,*";
		}else{
			frameset.cols = "0%,*";
		}
		try {
			...
			if(visibility == 'show'){
				element.className = element.className.replace("_hide", "_show");
			}else if(visibility == 'hide'){
				element.className = element.className.replace("_show", "_hide");
			}
		}catch(e){}
		return true;
	}
