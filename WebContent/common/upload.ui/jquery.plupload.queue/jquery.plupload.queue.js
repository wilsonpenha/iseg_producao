( function(c) {
	...
	function b(f, e) {
		...
		e.prepend('<div class="plupload_wrapper plupload_scroll"><div id="'
						+ f
						...
						+ a("Selecione os arquivos")
						+ '</div><div class="plupload_header_text">'
						+ a("Adicione os arquivos (Maximo 5m p/ imagens e 2mb para documentos), e click em iniciar upload.")
		...
		if (e) {
			this.each( function() {
						...
						function g() {
												...
												if (o.status == plupload.DONE) {
													if (o.target_name) {
														l += '<input type="hidden" name="'
																...
																+ n
																+ '_tmpname" value="'
																+ plupload.xmlEncode(o.target_name)
																+ '" />'
													}
													l += '<input type="hidden" name="'
															+ k
							...
							c("a.plupload_start", i).toggleClass(
											"plupload_disabled",
										...
										function(l, o) {
											...
											if (m) {
												...
												if (o.code == plupload.FILE_SIZE_ERROR) {
													alert(a("Error: Arquivo muito grande: ")
															+ m.name)
												}
												if (o.code == plupload.FILE_EXTENSION_ERROR) {
													alert(a("Error: Tipo de arquivo não suportado: ")
															+ m.name)
												}
												m.hint = n;
												c("#" + m.id).attr("class",
														"plupload_failed").find("a").css(
																"display",
																"block").attr(
																"title", n)
											}
