...
function setarPedidoValidado(){
	...
	submitForm($('#formEdit').get(0), getContextApp()+'/propostaImoveisFatura/efetivarProposta.action', false);
}
...
function salvarProposta(){
	if ($("#currentTabId").val()==2)
		$("#blnReloaded").val("false");
	if ($("#propostaId").val()=='')
		submitFormAjax('#formEdit', getContextApp()+'/propostaImoveisFatura/salvarEditar.action', '#allForms', '#saveProgress');
	else
		submitFormAjax('#formEdit', getContextApp()+'/propostaImoveisFatura/salvarEditar.action', '#tabPane'+$('#currentTabId').val(), '#saveProgress');
	setImovelAlterado(false);
	formNavigateAlterado = false;

}

function efetivarProposta(){
	submitFormAjax('#formEdit', getContextApp()+'/propostaImoveisFatura/efetivarProposta.action', '#allForms', '#saveProgress');
}
...
function excluirProposta(){
	...
	if(propostaId != "" && $('#statusOperacional').val() == '0'){
		...
		submitForm($('#formLookup').get(0), getContextApp()+'/propostaImoveisFatura/excluir.action', false);
	}
}
...
function carregaComboVistoriadora(seguradoraId) {
	...
	submitFormAjax('#formLookup', getContextApp()+'/propostaImoveisFatura/vistoriadoraPorSeguradora.action', '#divVistoriadora');
}
...
function carregaCliente() {
	...
	submitFormAjax('#formLookup', getContextApp()+'/propostaImoveisFatura/propostaImoveisDadosCliente.action', '#divDadosCliente');
	alterado=true;
	setImovelAlterado(true);
	setTimeout(function(){
...
function apagarDadosCliente(){
	...
	$("#gpTipoCliente").val($("#tipoCliente").val());
	submitFormAjax('#formLookup', getContextApp()+'/propostaImoveisFatura/novoDadosClienteForm.action', '#divDadosCliente');
}
...
function importarEnderecoCliente(){
	$("#cepRis").val($("#cepRes").val());
	$("#lograRis").val($("#lograRes").val());
	$("#compRis").val($("#compRes").val());
	$("#numeroRis").val($("#numeroRes").val());
	$("#foneRis").val($("#foneRes").val());
	$("#bairroRis").val($("#bairroRes").val());
	$("#cidadeRis").val($("#cidadeRes").val());
	$("#ufRis").val($("#ufRes").val());
	$("#faxRis").val($("#faxRes").val());
}
...
function carregaParcelas(){
	...
	if(alterado){
		...
		submitFormAjax('#formLookup', getContextApp()+'/propostaImoveisFatura/gerarParcelas.action', '#divParcelas');
	}
}
...
function calculaDemaisParcelas(){
	...
	var valorParcela = ((premioTotal-valor1aParcela)/qtdeParcelas).toFixed(2);
...
function alterarGrupoDeVenda(){
	...
	submitFormAjax('#formLookup', getContextApp()+'/propostaImoveisFatura/alterarGrupoVenda.action', '#divComissaoPagar');
}
...
function alterarGrupoDeVendaParcelaSelecionada(){
	...
	submitFormAjax('#formLookup', getContextApp()+'/propostaImoveisFatura/alterarGrupoVendaComissao.action', '#divRepasseComissao');
}
...
function inserirCobertura(){
	...
	submitFormAjaxAllArgs('#formCobertura', getContextApp()+'/propostaImoveisFatura/inserirCobertura.action', null, null, null, null, function(response){$('#tableCoberturas tbody').append(response);});
	$("#paramCobDescricaoCobertura").val('');
	$("#paramCobInformaFranquia").val('');
}

function novoItem(){
	submitFormAjax('#formEdit', getContextApp()+'/propostaImoveisFatura/novoItem.action', '#tabPane1');
}

function editarImovel(item){
	$("#itensSegurados").click();
	$("#itemCorrente").val(item);
	submitFormAjax('#formEdit', getContextApp()+'/propostaImoveisFatura/editarImovel.action', '#tabPane1');
}

function removerItem(item){
	$("#itensSegurados").click();
	$("#itemCorrente").val(item);
	submitFormAjax('#formEdit', getContextApp()+'/propostaImoveisFatura/removerItem.action', '#tabPane1');
}
...
function calculaPremioTotal(){
	...
	$("#premioTotal")[0].value = ((premioLiquido + custoApolice + iof) - desconto).toFixed(2);
	valida_numero($("#premioTotal")[0],15,2,'premioTotal');
...
function calculaDemaisParcelas(){
	...
	valida_numero($("#valorDemaisParcelas")[0],15,2,'valorDemaisParcelas');
}
...
function calculaTotalComissao(){
	...
	if(fator != 0 && premioLiquido != 0){
		$("#totalComissao").val((premioLiquido*fator/100).toFixed(2));
		valida_numero($("#totalComissao")[0],15,2,'totalComissao');
	}
}
...
function calculaFator(){
	...
	if(totalComissao != 0 && premioLiquido != 0){
		$("#fator").val((totalComissao*100/premioLiquido).toFixed(2));
		valida_numero($("#fator")[0],15,2,'fator');
	}
}
...
function setImovelAlterado(b){
	alterado = b;
	imovelAlterado = b;
	$('#imovelAlterado').val(b);
}
...
function validacaoGerarProposta(apolice){
	...
	if(!isEmpty('#pessoaFisicaId')){
		...
		//	fieldErrors += '- Inscrição Estadual do Segurado<br/>';
		//}
		//if(isEmpty('#inscricaoMunicipal')){
		//	fieldErrors += '- Inscrição Municipal do Segurado<br/>';
		//}
//		if(isEmpty('#atividadeComercial')){
//			fieldErrors += '- Atividade Comercial do Segurado<br/>';	
//		}
//		if(isEmpty('#dataFundacao')){
//			fieldErrors += '- Data de Fundação do Segurado<br/>';
//		}
//		if(isEmpty('#valorPatrimonio')){
//			fieldErrors += '- Valor Patrimonial do Segurado<br/>';
//		}
	}else{
