...
function importarEnderecoCliente(){
	if ($("#tipoCliente").val()==0){
		...
		$("#cepRis").val($("#cepCom").val());
		$("#lograRis").val($("#lograCom").val());
		$("#compRis").val($("#compCom").val());
		$("#numeroRis").val($("#numeroCom").val());
		$("#foneRis").val($("#foneCom").val());
		$("#bairroRis").val($("#bairroCom").val());
		$("#cidadeRis").val($("#cidadeCom").val());
		$("#ufRis").val($("#ufCom").val());
		$("#faxRis").val($("#faxCom").val());
	}	
}
...
function setarPedidoValidado(){
	...
	submitForm($('#formEdit').get(0), getContextApp()+'/propostaRiscosDiversos/efetivarProposta.action', false);
}
...
function salvarProposta(){
	submitFormAjax('#formEdit', getContextApp()+'/propostaRiscosDiversos/salvarEditar.action', '#allForms', '#saveProgress');
	setRiscoAlterado(false);
	formNavigateAlterado = false;

}

function efetivarProposta(){
	submitFormAjax('#formEdit', getContextApp()+'/propostaRiscosDiversos/efetivarProposta.action', '#allForms', '#saveProgress');
}
...
function excluirProposta(){
	...
	if(propostaId != "" && $('#statusOperacional').val() == '0'){
		...
		submitForm($('#formLookup').get(0), getContextApp()+'/propostaRiscosDiversos/excluir.action', false);
	}
}
...
function carregaComboVistoriadora(seguradoraId) {
	...
	submitFormAjax('#formLookup', getContextApp()+'/propostaRiscosDiversos/vistoriadoraPorSeguradora.action', '#divVistoriadora');
}
...
function carregaCliente() {
	...
	submitFormAjax('#formLookup', getContextApp()+'/propostaRiscosDiversos/propostaRiscosDiversosDadosCliente.action', '#divDadosCliente');
	alterado=true;
	setRiscoAlterado(true);
	setTimeout(function(){
...
function apagarDadosCliente(){
	...
	$("#gpTipoCliente").val($("#tipoCliente").val());
	submitFormAjax('#formLookup', getContextApp()+'/propostaRiscosDiversos/novoDadosClienteForm.action', '#divDadosCliente');
}
...
function carregaParcelas(){
	...
	if(alterado){
		...
		submitFormAjax('#formLookup', getContextApp()+'/propostaRiscosDiversos/gerarParcelas.action', '#divParcelas');
	}
}
...
function alterarGrupoDeVenda(){
	...
	submitFormAjax('#formLookup', getContextApp()+'/propostaRiscosDiversos/alterarGrupoVenda.action', '#tabPane4');
}
...
function calculaParcelas(){
	...
	if(valor1aParcela == 0 || qtdeParcelas == 1 || valor1aParcela > premioTotal){
		...
		$("#valorChequeRec").val($("#valor1aParcela").val());
	}else{
...
//		}
		...
		if(isEmpty('#bairroCom')){
			fieldErrors += '- Bairro<br/>';
		}
		if(isEmpty('#cidadeCom')){
			fieldErrors += '- Cidade<br/>';
		}
		if(isEmpty('#ufCom')){
			fieldErrors += '- Estado<br/>';
...
function inserirCobertura(){
	...
	submitFormAjaxAllArgs('#formCobertura', getContextApp()+'/propostaRiscosDiversos/inserirCobertura.action', null, null, null, null, function(response){$('#tableCoberturas tbody').append(response);});
	$("#paramCobDescricaoCobertura").val('');
	$("#paramCobInformaFranquia").val('');
}

function novoItem(){
	submitFormAjax('#formEdit', getContextApp()+'/propostaRiscosDiversos/novoItem.action', '#tabPane1');
}

function editarRisco(item){
	$("#itensSegurados").click();
	$("#itemCorrente").val(item);
	submitFormAjax('#formEdit', getContextApp()+'/propostaRiscosDiversos/editarRisco.action', '#tabPane1');
}

function removerItem(item){
	$("#itensSegurados").click();
	$("#itemCorrente").val(item);
	submitFormAjax('#formEdit', getContextApp()+'/propostaRiscosDiversos/removerItem.action', '#tabPane1');
}
