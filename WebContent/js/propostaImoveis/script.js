...
function setarPedidoValidado(){
	...
	submitForm($('#formEdit').get(0), getContextApp()+'/propostaImoveis/efetivarProposta.action', false);
}
...
function salvarProposta(){
	...
	if ($("#propostaId").val()=='')
		submitFormAjax('#formEdit', getContextApp()+'/propostaImoveis/salvarEditar.action', '#allForms', '#saveProgress');
	else
		submitFormAjax('#formEdit', getContextApp()+'/propostaImoveis/salvarEditar.action', '#tabPane'+$('#currentTabId').val(), '#saveProgress');
...
function efetivarProposta(){
	submitFormAjax('#formEdit', getContextApp()+'/propostaImoveis/efetivarProposta.action', '#allForms', '#saveProgress');
}
...
function excluirProposta(){
	...
	if(propostaId != "" && $('#statusOperacional').val() == '0'){
		...
		submitForm($('#formLookup').get(0), getContextApp()+'/propostaImoveis/excluir.action', false);
	}
}
...
function carregaComboVistoriadora(seguradoraId) {
	...
	submitFormAjax('#formLookup', getContextApp()+'/propostaImoveis/vistoriadoraPorSeguradora.action', '#divVistoriadora');
}
...
function carregaCliente() {
	...
	submitFormAjax('#formLookup', getContextApp()+'/propostaImoveis/propostaImoveisDadosCliente.action', '#divDadosCliente');
	alterado=true;
...
function apagarDadosCliente(){
	...
	$("#gpTipoCliente").val($("#tipoCliente").val());
	submitFormAjax('#formLookup', getContextApp()+'/propostaImoveis/novoDadosClienteForm.action', '#divDadosCliente');
}
...
function carregaParcelas(){
	...
	if(alterado){
		...
		submitFormAjax('#formLookup', getContextApp()+'/propostaImoveis/gerarParcelas.action', '#divParcelas');
	}
}
...
function alterarGrupoDeVenda(){
	...
	submitFormAjax('#formLookup', getContextApp()+'/propostaImoveis/alterarGrupoVenda.action', '#divComissaoPagar');
}
...
function inserirCobertura(){
	...
	submitFormAjaxAllArgs('#formCobertura', getContextApp()+'/propostaImoveis/inserirCobertura.action', null, null, null, null, function(response){$('#tableCoberturas tbody').append(response);});
	$("#paramCobDescricaoCobertura").val('');
	$("#paramCobInformaFranquia").val('');
}

function novoItem(){
	submitFormAjax('#formEdit', getContextApp()+'/propostaImoveis/novoItem.action', '#tabPane1');
}

function editarImovel(item){
	$("#itensSegurados").click();
	$("#itemCorrente").val(item);
	submitFormAjax('#formEdit', getContextApp()+'/propostaImoveis/editarImovel.action', '#tabPane1');
}

function removerItem(item){
	$("#itensSegurados").click();
	$("#itemCorrente").val(item);
	submitFormAjax('#formEdit', getContextApp()+'/propostaImoveis/removerItem.action', '#tabPane1');
}
