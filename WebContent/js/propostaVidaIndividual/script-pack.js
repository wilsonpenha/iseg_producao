...
$('#statusOperacional').val('81');submitForm($('#formEdit').get(0),getContextApp()+'/propostaIndividual/efetivarProposta.action',false);}
...
$('#statusOperacional').val('9');submitForm($('#formEdit').get(0),getContextApp()+'/propostaIndividual/efetivarProposta.action',false);}
...
$('#statusOperacional').val('10');submitForm($('#formEdit').get(0),getContextApp()+'/propostaIndividual/efetivarProposta.action',false);}
function salvarProposta(){if($("#propostaId").val()=='')
submitFormAjax('#formEdit',getContextApp()+'/propostaIndividual/salvarEditar.action','#allForms','#saveProgress');else
submitFormAjax('#formEdit',getContextApp()+'/propostaIndividual/salvarEditar.action','#tabPane'+$('#currentTabId').val(),'#saveProgress');formNavigateAlterado=false;}
function excluirProposta(){var propostaId=$('#propostaId').val();if(propostaId!=""&&$('#statusOperacional').val()=='0'){$("#gpPropostaId").val(propostaId);submitForm($('#formLookup').get(0),getContextApp()+'/propostaIndividual/excluir.action',false);}}
function carregaComboVistoriadora(seguradoraId){$("#gpSeguradoraId").val(seguradoraId);submitFormAjax('#formLookup',getContextApp()+'/propostaIndividual/vistoriadoraPorSeguradora.action','#divVistoriadora');}
function carregaSegurado(){$("#gpPropostaId").val($("#propostaId").val());$("#gpItemCorrente").val($("#itemCorrente").val());$("#gpClienteId").val($("#seguradoId").val());submitFormAjax('#formLookup',getContextApp()+'/propostaIndividual/propostaIndividualDadosSegurado.action','#divDadosSegurado');alterado=true;}
function carregaCliente(){$("#gpPropostaId").val($("#propostaId").val());$("#gpTipoCliente").val($("#tipoCliente").val());$("#gpClienteId").val($("#clienteId").val());$("#gpClienteCpf").val($("#clienteCpf").val());$("#gpClienteCnpj").val($("#clienteCnpj").val());submitFormAjax('#formLookup',getContextApp()+'/propostaIndividual/propostaIndividualDadosCliente.action','#divDadosCliente');alterado=true;setTimeout(function(){if(!isEmpty('#clienteId')){$("#cepRes").focus();}else
$("#clientePFNome").focus();},200);}
function apagarDadosCliente(){$('#formLookup #currentTabId').val('0');$("#gpTipoCliente").val($("#tipoCliente").val());submitFormAjax('#formLookup',getContextApp()+'/propostaIndividual/novoDadosClienteForm.action','#divDadosCliente');}
...
function removeBeneficiario(id){$("#comBeneficiarioId").val(id);$("#comSeguradoId").val($("#seguradoId").value);submitFormAjax('#formBeneficiario',getContextApp()+'/propostaIndividual/excluirBeneficiario.action','#divListaBeneficiarios');$("#beneficiarioNome").focus();}
function editarBeneficiario(id){$("#comBeneficiarioId").val(id);$("#beneficiarioNome").focus();submitFormAjax('#formBeneficiario',getContextApp()+'/propostaIndividual/editarBeneficiario.action','#divDadosBeneficiario');}
function inserirBeneficiario(){if($("#seguradoId").val()!=""){if($("#beneficiarioNome").val()==""){$("#beneficiarioNome").focus();alert("Informe o nome do Beneficiario!");return;}
$('#comItemCorrente').val($("#itemCorrente").val());$('#comPropostaId').val($("#propostaId").val());$('#comSeguradoId').val($("#seguradoId").val());$('#comBeneficiarioId').val($("#beneficiarioIndividualId").val());$('#comNome').val($("#beneficiarioNome").val());$('#comDataNascimento').val($("#beneficiarioDataNasc").val());$('#comSexo').val($("#beneficiarioSexo").val());$('#comParentesco').val($("#beneficiarioParentesco").val());$('#comPercentual').val($("#beneficiarioPercentual").val());submitFormAjax('#formBeneficiario',getContextApp()+'/propostaIndividual/inserirBeneficiario.action','#divListaBeneficiarios');$("#beneficiarioIndividualId").val("");$("#beneficiarioNome").val("");$("#beneficiarioDataNasc").val("");$("#beneficiarioSexo").val("");$("#beneficiarioParentesco").val("");$("#beneficiarioPercentual").val("");}else{alert("É necessario salvar o segurado primeiro!");}}
function apagarDadosCondutor(){$("#beneficiarioEmpresarialId").val("");$("#beneficiarioNome").val("");$("#beneficiarioDataNasc").val("");$("#beneficiarioSexo").val("");$("#beneficiarioParentesco").val("");$("#beneficiarioPercentual").val("");}
...
function replicarComissao(valor,index){for(var i=index+1;i<12;i++){$("#percentualComissao"+i).val(valor);}}
...
function alterarGrupoDeVenda(){$("#gpPropostaId").val($("#propostaId").val());$("#gpGrupoDeVendaId").val($("#grupoDeVendaId").val());submitFormAjax('#formLookup',getContextApp()+'/propostaIndividual/alterarGrupoVenda.action','#divComissaoPagar');}
...
function inserirCobertura(){$('#paramCobItemCorrente').val($("#itemCorrente").val());submitFormAjaxAllArgs('#formCobertura',getContextApp()+'/propostaIndividual/inserirCobertura.action',null,null,null,null,function(response){$('#tableCoberturas tbody').append(response);});$("#paramCobDescricaoCobertura").val('');$("#paramCobInformaFranquia").val('');}
function novoSegurado(){submitFormAjax('#formEdit',getContextApp()+'/propostaIndividual/novoSegurado.action','#tabPane1');}
function editarSegurado(item){$("#itensSegurados").click();$("#itemCorrente").val(item);submitFormAjax('#formEdit',getContextApp()+'/propostaIndividual/editarSegurado.action','#tabPane1');}
function removerSegurado(item){$("#itensSegurados").click();$("#itemCorrente").val(item);submitFormAjax('#formEdit',getContextApp()+'/propostaIndividual/removerSegurado.action','#tabPane1');}
function removerSegurado(){if($("#itemCorrente").val()==$("#itens").val()){$("#itemCorrente").val(0);submitFormAjax('#formEdit',getContextApp()+'/propostaIndividual/editarSegurado.action','#tabPane1');}else{submitFormAjax('#formEdit',getContextApp()+'/propostaIndividual/removerSegurado.action','#tabPane1');}}
function proximoItem(){var _itens=converte_numero($("#itens").val());var _item=converte_numero($("#itemCorrente").val());_item=_item+1;if(_item<_itens){$("#itemCorrente").val(_item);submitFormAjax('#formEdit',getContextApp()+'/propostaIndividual/editarSegurado.action','#tabPane1');}else
return false;}
function itemAnterior(){var _item=converte_numero($("#itemCorrente").val());_item=_item-1;if(_item>=0){$("#itemCorrente").val(_item);submitFormAjax('#formEdit',getContextApp()+'/propostaIndividual/editarSegurado.action','#tabPane1');}else
return false;}
...
if(isEmpty('#sexo')){fieldErrors+='- Sexo do Segurado<br/>';}}else{msgErrors+='- Não existe um Seguradora cadastrado para essa Proposta<br/>';}
...
if($('#tableBeneficiarios input[id^="beneficiarioIndividualId"]').add('#beneficiarioIndividualId').not('[value=""]').size()==0){if(isEmpty('#seguradoConjuge')){fieldErrors+='- Nome do Conjuge ou pelo menos um Beneficiario<br/>';}}
if($('#tableParcelas input[id^="ParcelasGeradas"]').add('#valorParcela0').not('[value=""]').size()==0){fieldErrors+='- Gerar parcelas<br/>';}
...
if($('#tableComissionadoItens tr').length<2){msgErrors+='- Não existe nenhum comissionado cadastrado para essa proposta<br/>';}
if(isNaN(apolice))
msg="validação do Proposta";else
msg=apolice;if(fieldErrors!=''){errors+='<b>Para a '+msg+', os seguintes campos devem ser preenchidos: </b><br/>'+fieldErrors;}
if(msgErrors!=''){errors+='<b>Existem condições pendentes que impossibilitam a '+msg+': </b><br/>'+msgErrors;}
if(errors!=''){$('div[id^="divMessagesTab"]').html('').html(errors);fieldErrors='';msgErrors='';errors='';return false;}
fieldErrors='';msgErrors='';errors='';return true;}
